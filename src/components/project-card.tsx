  ...
  return (
    ...
    >
      {image && (
        <Link
          href={href || "#"}
          className={cn("block cursor-pointer", getGradientClass())}
        >
          <Image
            src={image}
            alt={title}
            ...
            {links?.map((linkItem, idx) => (
              <Link href={linkItem?.href} key={idx} target="_blank">
                <Badge key={idx} className="flex gap-2 px-2 py-1 text-[10px]">
                  {linkItem.iconName === "github" || linkItem.type.toLowerCase().includes("source") || linkItem.type.toLowerCase().includes("github") ? (
